...
현재 안정 방식은 독립 Generative View 기반 `TOP_VIEW` / `RIGHT_VIEW` 생성이다. STEP 5A에서는 CATIA API 기반 Projection View 생성을 실험 중이며, API 방식이 완전히 검증되기 전까지 독립 Generative View 방식은 fallback으로 유지한다.
...
- STEP 5A CATIA API Projection View 실험 경로 추가
- output 폴더로 `활성문서명_도면사이즈.CATDrawing` SaveAs
- CATIA COM 예외 상세 로그 출력

## 다음 방향

- STEP 5A: CATIA API 기반 Projection View 생성 검증 지속
...
- CATIA API Projection View가 완전히 검증되기 전까지 독립 Generative View fallback을 제거하지 않는다.